...
export const VISUAL_EMPTY_COPY = {
  routes:
    "Маршруты готовятся к публикации. Можно написать — подберём формат под ваш визит, или заглянуть в материалы о городе.",
  routePoints:
    "Здесь будет снимок точки — пока опишем место словами, а фото добавим, когда найдём удачный ракурс.",
  photos:
    "Архив визуальной истории Иркутска наполняется: старые и современные кадры, улицы и места. Можно предложить своё фото.",
  photosFiltered:
    "По этим фильтрам ничего не нашлось. Попробуйте другой период, улицу или категорию.",
  souvenirs:
    "Коллекция локальных вещей пополняется — без вымышленных остатков. Напишите, если хотите узнать первыми.",
  souvenirsMaker:
  ...
  events:
    "Календарь событий готовится. Ближайших дат пока нет — анонсы появляются в Telegram, а форму можно использовать, чтобы предложить событие.",
  shop:
