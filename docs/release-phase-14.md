...
| `allowDemoFallback()` — только dev без БД | На проде не показывать демо-фото/статьи/маршруты |
| Удалены legacy `/shop`, Stripe checkout | Сувениры через заявки, не оплата |
| Редирект `/excursions` в `next.config` | Постоянный 308 вместо page-level redirect |
| Скрытие фейковых статов на главной | Блок доверия без «100+ гостей» по умолчанию |
...
| `npm run check:prod` | Быстрая проверка URL после деплоя |
| Чистая пересборка на проде | Исправлены 500 на `/business`, `/admin` |

## 3. Что осталось на будущее

- Настроить Resend для email-уведомлений о заявках (`RESEND_API_KEY` на VPS)
...
- Поля `stripePriceId` в CMS — legacy, можно убрать из схемы позже
